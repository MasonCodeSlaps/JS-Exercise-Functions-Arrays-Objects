 ...
 * it will return `This is a Lincoln Navigator`.
 */
function getCarInfoById(carArr , id) {
  const idArr = [];
...
function getModelYears(modelYears) {
  ...
  for (let i = 0; i < modelYears.length; i++){
    if(modelYears[i].car_year === modelYears.car_year)
    carYearsArr.push(modelYears[i]);
    // console.log(carYearsArr[i]);
  }
 ...
 */
function getOlderCars(oldCars , year) {
  const olderCarsArr =[]
  
  for (let i = 0; i < oldCars.length; i++){
    if(oldCars[i].car_year <= 1970)
    olderCarsArr.push(oldCars[i]);
  }
// console.log(oldCars);
}
...
function getGermanCars(german) {
  ...
  let germanCars = []
   ...
   if(german[i].car_make === 'Audi' || 
   german[i].car_make === 'Mercedes-Benz' || 
   german[i].car_make === 'Volkswagen' || 
   german[i].car_make === 'BMW'){
   germanCars.push(german[i]);
     // console.log(german[i]);
   }
  return germanCars;
 }
